...
const Feature = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-col md:border-r py-10 relative group/feature border-slate-200 dark:border-slate-800",
        index % 2 === 0 && "md:border-l border-slate-200 dark:border-slate-800",
        isEvenRow && "md:border-b border-slate-200 dark:border-slate-800"
      )}
    >
      {isEvenRow && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-linear-to-t from-slate-50 dark:from-slate-900/50 to-transparent pointer-events-none" />
      )}
      {!isEvenRow && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-linear-to-b from-slate-50 dark:from-slate-900/50 to-transparent pointer-events-none" />
      )}
      <div className="mb-4 relative z-10 px-10 text-slate-600 dark:text-slate-400">
        {icon}
      </div>
      {subtitle && (
        <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 relative z-10 px-10">
          {subtitle}
        </div>
      )}
      <div className="text-xl font-black mb-2 relative z-10 px-10">
        <div className="absolute left-0 inset-y-0 h-6 group-hover/feature:h-8 w-1 rounded-tr-full rounded-br-full bg-slate-300 dark:bg-slate-700 group-hover/feature:bg-slate-900 dark:group-hover/feature:bg-white transition-all duration-200 origin-center" />
        <span className="group-hover/feature:translate-x-2 transition duration-200 inline-block text-slate-900 dark:text-white">
          {title}
        </span>
      </div>
      <p className="text-sm text-slate-600 dark:text-slate-400 max-w-md relative z-10 px-10 leading-relaxed">
        {description}
      </p>
    </div>
  );
